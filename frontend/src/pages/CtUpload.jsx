...
const CtUpload = () => {
  ...
  useEffect(() => {
    ...
    const loadModels = async () => {
      try {
        ...
        if (navigator.onLine) {
          const response = await uploadService.getCtModels();
          if (response.data && response.data.models) {
            ...
            localStorage.setItem('ctModels', JSON.stringify(modelsList));
          ...
          const cachedModels = localStorage.getItem('ctModels');
          if (cachedModels) {
    ...
    const loadPendingUploads = () => {
      const savedUploads = localStorage.getItem('pendingCtUploads');
      if (savedUploads) {
  ...
  const processPendingUploads = async () => {
    ...
    for (let i = 0; i < currentPending.length; i++) {
      ...
      try {
        ...
        localStorage.setItem('pendingCtUploads', JSON.stringify(updatedPending));
  ...
  const uploadAndAnalyzeFile = async (file, modelId) => {
    ...
    try {
      ...
      const response = await uploadService.uploadCt(file, modelId, (progressEvent) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-blue-800 mb-2">CT Scan Upload</h1>
        <p className="text-gray-600">
          Upload and analyze CT scans using our advanced AI models
        </p>
      </div>
      ...
      >
        ...
        <p className="text-gray-600 mb-2">
          Drag and drop your CT scan files here, or{' '}
          <button
