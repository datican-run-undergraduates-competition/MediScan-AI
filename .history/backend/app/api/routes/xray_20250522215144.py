"""
API routes for X-ray image analysis.
"""
...
@router.get("/models")
async def list_xray_models(current_user = Depends(get_current_active_user)):
    """
    List available X-ray analysis models.
    """
    # Get all models
    all_models = inference_engine.get_available_models()
    
    # Filter for X-ray models based on metadata
    xray_models = {}
    for name, model_info in all_models.items():
        # Check if model is for X-ray analysis
        if (model_info.get("info", {}).get("metadata", {}).get("modality", "").lower() == "xray" or
            "xray" in name.lower()):
            xray_models[name] = model_info
    
    return {"models": xray_models}
...
async def analyze_xray(
    ...
    """
    Analyze an X-ray image using the specified model.
    """
    try:
        # Read image file
        ...
        if not model_name:
            ...
            xray_models = {
                ...
                if (info.get("info", {}).get("metadata", {}).get("modality", "").lower() == "xray" or
                   "xray" in name.lower())
            }
            ...
            if not xray_models:
                raise HTTPException(
                    ...
                    detail="No X-ray models available"
                )
            
            # Use the first available model
            model_name = next(iter(xray_models.keys()))
        ...
        raise HTTPException(
            ...
            detail=f"Error processing X-ray: {str(e)}"
        ) 
